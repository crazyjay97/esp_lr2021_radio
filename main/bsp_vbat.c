/*
 * Battery / external-supply voltage sense for the L-LRMAM36-FANN4-DK01.
 *
 * VBAT_ADC is brought out to GPIO11 (ADC2 channel 0). On the board the battery
 * rail passes through a 1M/1M divider (R114/R115), so the pin only ever sees
 * VBAT/2. bsp_vbat_read_mv() reads the pin and multiplies by the divider ratio
 * so the caller gets the real supply voltage in millivolts.
 *
