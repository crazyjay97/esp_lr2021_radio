...
static esp_err_t i2s_init(uint32_t sample_rate_hz, uint32_t dma_desc_num)
{
    ...
    // Keep the node's deeper DMA ring for capture/JPEG contention, but let the
    // gateway use the default depth. In gateway intercom mode video is stopped,
    // so allocating 24 descriptors for both TX and RX only starves Wi-Fi's
    // internal-RAM beacon buffers. At 16 kHz, 240 frames per descriptor is 15 ms.
