...
/*
 * Build a DNS response that points every A query at the AP IP. We copy the
 * original question back and append one answer record pointing at get_ap_ip().
 * Returns the total response length, or 0 if the request is not usable.
 */
...
/*
 * HTTP 404 handler: redirect anything we do not serve to the gallery. The OS
 * connectivity probes hit unknown URLs, get a 302, and open the captive popup.
 */
